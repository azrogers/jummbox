...
export class FilterEditor {
	...
	private _filterSettings: FilterSettings = new FilterSettings();
	...
	private _whenMouseOver = (event: MouseEvent): void => {
		...
		if (!this._larger) this._controlPointPath.style.setProperty("fill", "currentColor");
	};
		...
		if (this._mouseDown && this._doc.lastChangeWas(this._dragChange) && this._dragChange != null) {
			if (!this._addingPoint && !this._mouseDragging && !this._touchMode) {
				if (this._selectedIndex < this._filterSettings.controlPointCount && this._selectedIndex != -1) {
					const point: FilterControlPoint = this._filterSettings.controlPoints[this._selectedIndex];
					let change: ChangeFilterAddPoint = new ChangeFilterAddPoint(
						this._doc,
						this._filterSettings,
						point,
						this._selectedIndex,
	...
	private _updatePath(): void {
		...
		for (let i: number = 0; i < this._filterSettings.controlPointCount; i++) {
			...
			if (
				(this._selectedIndex == i ||
					(this._addingPoint && this._mouseDown && i == this._filterSettings.controlPointCount - 1)) &&
				...
				this._label.textContent =
					...
					1 +
					": " +
					Config.filterTypeNames[point.type] +
	...
	public swapSubfilterIndices(newIndex: number) {
		...
		if (newIndex >= this._filterSettings.controlPointCount) return;
	...
	public render(activeMods: boolean = false): void {
		...
		if (displayMods) this._controlPointPath.style.setProperty("fill", `${ColorConfig.overwritingModSlider}`);
		else if (!this._larger) this._controlPointPath.style.setProperty("fill", "currentColor");
