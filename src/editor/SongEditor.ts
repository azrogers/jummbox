...
function buildPresetOptions(isNoise: boolean, idSet: string): HTMLSelectElement {
	...
	if (isNoise) {
		menu.appendChild(
			option({ value: InstrumentType.noise }, EditorConfig.valueToPreset(InstrumentType.noise)!.name)
		);
		menu.appendChild(
			option({ value: InstrumentType.spectrum }, EditorConfig.valueToPreset(InstrumentType.spectrum)!.name)
		);
		menu.appendChild(
			option({ value: InstrumentType.drumset }, EditorConfig.valueToPreset(InstrumentType.drumset)!.name)
		);
	} else {
		...
		menu.appendChild(
			option({ value: InstrumentType.harmonics }, EditorConfig.valueToPreset(InstrumentType.harmonics)!.name)
		);
		menu.appendChild(
			option(
...
class CustomChipCanvas {
	...
	public redrawCanvas(): void {
		...
		// Waveform
		ctx.fillStyle = ColorConfig.getComputedChannelColor(Doc.song, Doc.channel).primaryNote;

		for (let x: number = 0; x < 64; x++) {
			var y: number =
				Doc.song.channels[Doc.channel].instruments[Doc.getCurrentInstrument()].customChipWave[x] + 26;
...
export class SongEditor {
	...
	private readonly _keyboardLayout: KeyboardLayout = new KeyboardLayout(Doc);
	private readonly _patternEditorPrev: PatternEditor = new PatternEditor(false, -1);
	private readonly _patternEditor: PatternEditor = new PatternEditor(true, 0);
	private readonly _patternEditorNext: PatternEditor = new PatternEditor(false, 1);
	private readonly _trackEditor: TrackEditor = new TrackEditor(Doc, this);
	private readonly _muteEditor: MuteEditor = new MuteEditor(Doc, this);
	private readonly _loopEditor: LoopEditor = new LoopEditor(Doc);
	private readonly _piano: Piano = new Piano(Doc);
	private readonly _octaveScrollBar: OctaveScrollBar = new OctaveScrollBar(Doc, this._piano);
	...
	private readonly _pauseButton: HTMLButtonElement = button(
		{ class: "pauseButton", style: "display: none;", type: "button", title: "Pause (Space)" },
		"Pause"
	);
	private readonly _recordButton: HTMLButtonElement = button(
		{ class: "recordButton", style: "display: none;", type: "button", title: "Record (Ctrl+Space)" },
		span("Record")
	);
	private readonly _stopButton: HTMLButtonElement = button(
		{ class: "stopButton", style: "display: none;", type: "button", title: "Stop Recording (Space)" },
		"Stop Recording"
	);
	...
	private readonly _panDelayRow: HTMLElement = div(
		{ class: "selectRow" },
		span({ class: "tip", style: "margin-left:10px;", onclick: () => this._openPrompt("panDelay") }, "Delay:"),
	...
	private readonly _fadeInOutEditor: FadeInOutEditor = new FadeInOutEditor(Doc);
	...
	private readonly _clicklessTransitionRow: HTMLElement = div(
		{ class: "selectRow" },
		span(
			{ class: "tip", style: "margin-left:10px;", onclick: () => this._openPrompt("clicklessTransition") },
			"Clickless:"
		),
	...
	private readonly _eqFilterTypeRow: HTMLElement = div(
		...
		span(
			{ style: "font-size: x-small;", class: "tip", onclick: () => this._openPrompt("filterType") },
			"EQ Filt.Type:"
		),
		div({ class: "instrument-bar" }, this._eqFilterSimpleButton, this._eqFilterAdvancedButton)
	);
	private readonly _eqFilterEditor: FilterEditor = new FilterEditor(Doc);
	...
	private readonly _noteFilterEditor: FilterEditor = new FilterEditor(Doc, true);
	...
	private readonly _pulseWidthRow: HTMLDivElement = div(
		{ class: "selectRow" },
		span({ class: "tip", onclick: () => this._openPrompt("pulseWidth") }, "Pulse Width:"),
	...
	private readonly _pitchShiftSlider: Slider = new Slider(
		...
		Doc,
		(oldValue: number, newValue: number) => new ChangePitchShift(Doc, oldValue, newValue),
		true
	);
	...
	private readonly _detuneSlider: Slider = new Slider(
		...
		Doc,
		(oldValue: number, newValue: number) => new ChangeDetune(Doc, oldValue, newValue),
		true
	);
	...
	private readonly _detuneSliderRow: HTMLDivElement = div(
		...
		div(
			{},
			span(
				{ class: "tip", style: "height:1em; font-size: smaller;", onclick: () => this._openPrompt("detune") },
				"Detune: "
			),
	...
	private readonly _stringSustainRow: HTMLDivElement = div(
		{ class: "selectRow" },
		span({ class: "tip", onclick: () => this._openPrompt("stringSustain") }, "Sustain:"),
	...
	private readonly _unisonSelectRow: HTMLElement = div(
		...
		div({ class: "selectContainer" }, this._unisonSelect)
	);
	...
	private readonly _chordSelectRow: HTMLElement = div(
		{ class: "selectRow" },
		span({ class: "tip", onclick: () => this._openPrompt("chords") }, "Chords:"),
		this._chordDropdown,
		div({ class: "selectContainer" }, this._chordSelect)
	);
	...
	private readonly _arpeggioSpeedRow: HTMLElement = div(
		{ class: "selectRow" },
		span({ class: "tip", style: "margin-left:10px;", onclick: () => this._openPrompt("arpeggioSpeed") }, "Speed:"),
	...
	private readonly _twoNoteArpRow: HTMLElement = div(
		{ class: "selectRow" },
		span(
			{ class: "tip", style: "margin-left:10px;", onclick: () => this._openPrompt("twoNoteArpeggio") },
			"Fast Two-Note:"
		),
		this._twoNoteArpBox
	);
	...
	private readonly _vibratoDepthRow: HTMLElement = div(
		{ class: "selectRow" },
		span({ class: "tip", style: "margin-left:10px;", onclick: () => this._openPrompt("vibratoDepth") }, "Depth:"),
	...
	private readonly _vibratoSpeedRow: HTMLElement = div(
		{ class: "selectRow" },
		span({ class: "tip", style: "margin-left:10px;", onclick: () => this._openPrompt("vibratoSpeed") }, "Speed:"),
	...
	private readonly _vibratoDelayRow: HTMLElement = div(
		{ class: "selectRow" },
		span({ class: "tip", style: "margin-left:10px;", onclick: () => this._openPrompt("vibratoDelay") }, "Delay:"),
	...
	private readonly _vibratoTypeSelectRow: HTMLElement = div(
		{ class: "selectRow" },
		span({ class: "tip", style: "margin-left:10px;", onclick: () => this._openPrompt("vibratoType") }, "Type:"),
	...
	private readonly _spectrumEditor: SpectrumEditor = new SpectrumEditor(Doc, null);
	private readonly _spectrumRow: HTMLElement = div(
		{ class: "selectRow" },
		span({ class: "tip", onclick: () => this._openPrompt("spectrum") }, "Spectrum:"),
		this._spectrumEditor.container
	);
	private readonly _harmonicsEditor: HarmonicsEditor = new HarmonicsEditor(Doc);
	private readonly _harmonicsRow: HTMLElement = div(
		{ class: "selectRow" },
		span({ class: "tip", onclick: () => this._openPrompt("harmonics") }, "Harmonics:"),
		this._harmonicsEditor.container
	);
	private readonly _envelopeEditor: EnvelopeEditor = new EnvelopeEditor(Doc);
	...
	private readonly _instrumentCopyButton: HTMLButtonElement = button(
		{ style: "max-width:86px; width: 86px;", class: "copyButton" },
		[
			"Copy",
			// Copy icon:
			SVG.svg(
	...
	private readonly _instrumentPasteButton: HTMLButtonElement = button(
		{ style: "max-width:86px;", class: "pasteButton" },
		[
			"Paste",
			// Paste icon:
			SVG.svg(
	...
	private readonly _customWaveDrawCanvas: CustomChipCanvas = new CustomChipCanvas(
		...
		Doc,
		(newArray: Float32Array) => new ChangeCustomWave(Doc, newArray)
	);
    ...
    */
	...
	private readonly _jumpToModIndicator: SVGElement = SVG.svg(
		{ style: "width: 92%; height: 1.3em; flex-shrink: 0; position: absolute;", viewBox: "0 0 200 200" },
		[
			SVG.path({
	...
	public readonly _barScrollBar: BarScrollBar = new BarScrollBar(Doc, this._trackAndMuteContainer);
	...
	constructor() {
		Doc.notifier.watch(this.whenUpdated);
		new MidiInputHandler(Doc);
		...
		this._scaleSelect.appendChild(
			optgroup({ label: "Edit" }, option({ value: "forceScale" }, "Snap Notes To Scale"))
		);
		...
		for (let i: number = 0; i < Config.operatorCount; i++) {
			...
			const amplitudeSlider: Slider = new Slider(
				...
				(oldValue: number, newValue: number) =>
					new ChangeOperatorAmplitude(Doc, operatorIndex, oldValue, newValue),
				false
			);
			...
			const waveformDropdownHint: HTMLSpanElement = span(
				{ class: "tip", style: "margin-left: 10px;", onclick: () => this._openPrompt("operatorWaveform") },
				"Wave:"
			);
			const waveformPulsewidthSlider: Slider = new Slider(
				...
				Doc,
				(oldValue: number, newValue: number) =>
					new ChangeOperatorPulseWidth(Doc, operatorIndex, oldValue, newValue),
				true
			);
			...
			waveformSelect.addEventListener("change", () => {
				Doc.record(new ChangeOperatorWaveform(Doc, operatorIndex, waveformSelect.selectedIndex));
			});

			frequencySelect.addEventListener("change", () => {
				Doc.record(new ChangeOperatorFrequency(Doc, operatorIndex, frequencySelect.selectedIndex));
			});
		}
		...
		for (let i: number = Config.drumCount - 1; i >= 0; i--) {
			...
			const spectrumEditor: SpectrumEditor = new SpectrumEditor(Doc, drumIndex);
			...
			envelopeSelect.addEventListener("change", () => {
				Doc.record(new ChangeDrumsetEnvelope(Doc, drumIndex, envelopeSelect.selectedIndex));
			});
		...
		for (let mod: number = 0; mod < Config.modCount; mod++) {
			...
			let modSetRow: HTMLDivElement = div(
				...
				span({ class: "tip", onclick: () => this._openPrompt("modSet") }, "Setting: "),
				span(
					{ class: "tip", style: "font-size:x-small;", onclick: () => this._openPrompt("modSetInfo" + mod) },
					"?"
				),
			...
			let modFilterRow: HTMLDivElement = div(
				...
				span({ class: "tip", onclick: () => this._openPrompt("modFilter" + mod) }, "Target: "),
			...
			let modTarget: SVGElement = SVG.svg(
				{ style: "transform: translate(0px, 1px);", width: "1.5em", height: "1em", viewBox: "0 0 200 200" },
				[
					SVG.path({
						d: "M90 155 l0 -45 -45 0 c-25 0 -45 -4 -45 -10 0 -5 20 -10 45 -10 l45 0 0 -45 c0 -25 5 -45 10 -45 6 0 10 20 10 45 l0 45 45 0 c25 0 45 5 45 10 0 6 -20 10 -45 10 l -45 0 0 45 c0 25 -4 45 -10 45 -5 0 -10 -20 -10 -45z"
					}),
		...
		this._instrumentVolumeSliderInputBox.addEventListener("input", () => {
			Doc.record(
				new ChangeVolume(
					Doc,
					Doc.song.channels[Doc.channel].instruments[Doc.getCurrentInstrument()].volume,
		...
		this._detuneSliderInputBox.addEventListener("input", () => {
			Doc.record(
				new ChangeDetune(
					Doc,
					Doc.song.channels[Doc.channel].instruments[Doc.getCurrentInstrument()].detune,
					Math.min(
		...
		this._customWaveDraw.addEventListener("input", () => {
			Doc.record(new ChangeCustomWave(Doc, this._customWaveDrawCanvas.newArray));
		});
		this._twoNoteArpBox.addEventListener("input", () => {
			Doc.record(new ChangeFastTwoNoteArp(Doc, this._twoNoteArpBox.checked));
		});
		this._clicklessTransitionBox.addEventListener("input", () => {
			Doc.record(new ChangeClicklessTransition(Doc, this._clicklessTransitionBox.checked));
		});
		this._aliasingBox.addEventListener("input", () => {
			Doc.record(new ChangeAliasing(Doc, this._aliasingBox.checked));
		});
	...
	private _modSliderUpdate(): void {
		if (!Doc.synth.playing) {
			...
			let instrument: number = Doc.getCurrentInstrument();
			const anyModActive: boolean = Doc.synth.isAnyModActive(Doc.channel, instrument);
			...
			if (anyModActive) {
				...
				for (let setting: number = 0; setting < Config.modulators.length; setting++) {
					...
					let slider: Slider | null = this._getSliderForModSetting(setting);
					if (slider != null) {
						this._newShowModSliders[setting] = updateModSlider(
							this,
							slider,
							setting,
							Doc.channel,
		...
		if (promptName) {
			switch (promptName) {
				...
				case "limiterSettings":
					this.prompt = new LimiterPrompt(Doc, this);
					break;
				case "customChipSettings":
					this.prompt = new CustomChipPrompt(Doc, this);
					break;
				case "customEQFilterSettings":
					this.prompt = new CustomFilterPrompt(Doc, this, false);
					break;
				case "customNoteFilterSettings":
					this.prompt = new CustomFilterPrompt(Doc, this, true);
					break;
				case "theme":
	...
	private _refocusStageNotEditing = (): void => {
		if (!this._patternEditor.editingModLabel) this.mainLayer.focus({ preventScroll: true });
	};
	...
	public whenUpdated = (): void => {
		...
		Doc.trackVisibleChannels = Math.floor((trackBounds.bottom - trackBounds.top - 30) / Doc.getChannelHeight());
		for (
			let i: number = Doc.song.pitchChannelCount + Doc.song.noiseChannelCount;
			i < Doc.song.channels.length;
			i++
		) {
			const channel: Channel = Doc.song.channels[i];
		...
		this._barScrollBar.container.style.display = Doc.song.barCount > Doc.trackVisibleBars ? "" : "none";
		this._volumeBarBox.style.display = Doc.prefs.displayVolumeBar ? "" : "none";

		if (Doc.getFullScreen()) {
			const semitoneHeight: number = this._patternEditorRow.clientHeight / Doc.getVisiblePitchCount();
			const targetBeatWidth: number = semitoneHeight * 5;
			const minBeatWidth: number = this._patternEditorRow.clientWidth / (Doc.song.beatsPerBar * 3);
			const maxBeatWidth: number = this._patternEditorRow.clientWidth / (Doc.song.beatsPerBar + 2);
			...
			this._zoomInButton.style.display = Doc.channel < Doc.song.pitchChannelCount ? "" : "none";
			this._zoomOutButton.style.display = Doc.channel < Doc.song.pitchChannelCount ? "" : "none";
		...
		const optionCommands: ReadonlyArray<string> = [
			...
			(prefs.defaultScale == Doc.song.scale ? "✓ " : "　") + "Use Current Scale as Default",
		...
		const colors: ChannelColors = ColorConfig.getChannelColor(Doc.song, Doc.channel);
		...
		setSelectedValue(this._scaleSelect, Doc.song.scale);
		this._scaleSelect.title = Config.scales[Doc.song.scale].realName;
		setSelectedValue(this._keySelect, Config.keys.length - 1 - Doc.song.key);
		this._tempoSlider.updateValue(Math.max(0, Math.round(Doc.song.tempo)));
		this._tempoStepper.value = Math.round(Doc.song.tempo).toString();
		this._songTitleInputBox.updateValue(Doc.song.title);
		...
		setSelectedValue(this._rhythmSelect, Doc.song.rhythm);
		...
		if (!Doc.song.getChannelIsMod(Doc.channel)) {
			...
			if (Doc.song.channels[Doc.channel].name == "") {
				...
				this._instrumentSettingsTextRow.textContent = Doc.song.channels[Doc.channel].name;
			}
			...
			this._usageCheck(Doc.channel, instrumentIndex);
			...
			if (effectsIncludeChord(instrument.effects)) {
				...
				this._chordDropdownGroup.style.display =
					instrument.chord == Config.chords.dictionary["arpeggio"].index && this._openChordDropdown
						? ""
						: "none";
			...
			this._instrumentSettingsGroup.style.color = ColorConfig.getChannelColor(Doc.song, Doc.channel).primaryNote;
		...
		// Options for mod channel
			...
			this._modulatorGroup.style.color = ColorConfig.getChannelColor(Doc.song, Doc.channel).primaryNote;
			...
			for (let mod: number = 0; mod < Config.modCount; mod++) {
				...
				if (
					...
					this._modChannelBoxes[mod].children.length !=
				...
				let channel: Channel = Doc.song.channels[modChannel];
				...
				if (channel.bars[Doc.bar] > 0) {
					let usedInstruments: number[] = channel.patterns[channel.bars[Doc.bar] - 1].instruments;
				...
				if (instrument.modChannels[mod] != -2) {
					...
					// Populate mod setting options for instrument scope.
						...
						for (let i: number = 0; i < instrumentCandidates.length; i++) {
							...
							if (channel.instruments[instrumentIndex].eqFilterType) anyInstrumentSimpleEQ = true;
							...
							if (effectsIncludeNoteFilter(channel.instruments[instrumentIndex].effects)) {
								...
								if (channel.instruments[instrumentIndex].noteFilterType) anyInstrumentSimpleNote = true;
	...
	public updatePlayButton = (): void => {
		if (
			this._renderedIsPlaying != Doc.synth.playing ||
			this._renderedIsRecording != Doc.synth.recording ||
			this._renderedShowRecordButton != Doc.prefs.showRecordButton ||
			...
			} else if (Doc.synth.playing) {
				this._pauseButton.style.display = "";
			} else if (Doc.prefs.showRecordButton) {
	...
	private _usageCheck(channelIndex: number, instrumentIndex: number): void {
		...
		if (channelIndex < Doc.song.pitchChannelCount + Doc.song.noiseChannelCount) {
			for (
				...
				modChannelIdx < Doc.song.channels.length;
				modChannelIdx++
			) {
				const modChannel: Channel = Doc.song.channels[modChannelIdx];
				const patternIdx = modChannel.bars[Doc.bar];
				if (patternIdx > 0) {
		...
		if (channel.bars[Doc.bar] != 0) {
			let lowestSelX: number = Math.min(Doc.selection.boxSelectionX0, Doc.selection.boxSelectionX1);
			let highestSelX: number = Math.max(Doc.selection.boxSelectionX0, Doc.selection.boxSelectionX1);
			let lowestSelY: number = Math.min(Doc.selection.boxSelectionY0, Doc.selection.boxSelectionY1);
			let highestSelY: number = Math.max(Doc.selection.boxSelectionY0, Doc.selection.boxSelectionY1);

			for (let i: number = 0; i < Doc.song.barCount; i++) {
				// Check for this exact bar in another place, but only count it if it's not within the selection
				if (
					channel.bars[i] == channel.bars[Doc.bar] &&
					i != Doc.bar &&
					(i < lowestSelX || i > highestSelX || Doc.channel < lowestSelY || Doc.channel > highestSelY)
				) {
					patternUsed = true;
					i = Doc.song.barCount;
				}
		...
		switch (event.keyCode) {
			...
			case 68: // d
				...
				if (needControlForShortcuts == (event.ctrlKey || event.metaKey)) {
					Doc.selection.duplicatePatterns();
					event.preventDefault();
				}
				break;
	...
	public _volumeUpdate = (): void => {
		...
		if (Doc.song.outVolumeCap > this.outVolumeHistoricCap) {
			this.outVolumeHistoricCap = Doc.song.outVolumeCap;
			this.outVolumeHistoricTimer = 50;
		}

		if (Doc.song.outVolumeCap != this.lastOutVolumeCap) {
			this.lastOutVolumeCap = Doc.song.outVolumeCap;
			this._animateVolume(Doc.song.outVolumeCap, this.outVolumeHistoricCap);
		}
	};
	...
	private _setVolumeSlider = (): void => {
		Doc.setVolume(Number(this._volumeSlider.input.value));
	};
	...
	private _copyInstrument = (): void => {
		...
		instrumentCopy["isDrum"] = Doc.song.getChannelIsNoise(Doc.channel);
	...
	private _pasteInstrument = (): void => {
		...
		if (instrumentCopy != null && instrumentCopy["isDrum"] == Doc.song.getChannelIsNoise(Doc.channel)) {
			Doc.record(new ChangePasteInstrument(Doc, instrument, instrumentCopy));
		}
		this.refocusStage();
	};
	...
	private _switchEQFilterType(toSimple: boolean) {
		...
		if (instrument.eqFilterType != toSimple) {
			Doc.record(new ChangeEQFilterType(Doc, instrument, toSimple));
		}
	}
	...
	private _switchNoteFilterType(toSimple: boolean) {
		...
		if (instrument.noteFilterType != toSimple) {
			Doc.record(new ChangeNoteFilterType(Doc, instrument, toSimple));
		}
	}

	private _randomPreset(): void {
		const isNoise: boolean = Doc.song.getChannelIsNoise(Doc.channel);
		Doc.record(new ChangePreset(Doc, pickRandomPresetValue(isNoise)));
	}

	private _randomGenerated(): void {
		Doc.record(new ChangeRandomGeneratedInstrument(Doc));
	}

	private _whenSetTempo = (): void => {
		Doc.record(new ChangeTempo(Doc, -1, parseInt(this._tempoStepper.value) | 0));
	};
	...
	private _whenSetScale = (): void => {
		if (isNaN(<number>(<unknown>this._scaleSelect.value))) {
			switch (this._scaleSelect.value) {
				case "forceScale":
					Doc.selection.forceScale();
					break;
			}
			Doc.notifier.changed();
		} else {
			Doc.record(new ChangeScale(Doc, this._scaleSelect.selectedIndex));
		}
	};
	...
	private _whenSetKey = (): void => {
		if (isNaN(<number>(<unknown>this._keySelect.value))) {
			switch (this._keySelect.value) {
				case "detectKey":
					Doc.record(new ChangeDetectKey(Doc));
					break;
			}
			Doc.notifier.changed();
		} else {
			Doc.record(new ChangeKey(Doc, Config.keys.length - 1 - this._keySelect.selectedIndex));
		}
	};
	...
	private _whenSetRhythm = (): void => {
		if (isNaN(<number>(<unknown>this._rhythmSelect.value))) {
			...
			Doc.record(new ChangeRhythm(Doc, this._rhythmSelect.selectedIndex));
		}
	};
	...
	private _setPreset(preset: string): void {
		if (isNaN(<number>(<unknown>preset))) {
			...
			Doc.record(new ChangePreset(Doc, parseInt(preset)));
		}
	}

	private _whenSetFeedbackType = (): void => {
		Doc.record(new ChangeFeedbackType(Doc, this._feedbackTypeSelect.selectedIndex));
	};

	private _whenSetAlgorithm = (): void => {
		Doc.record(new ChangeAlgorithm(Doc, this._algorithmSelect.selectedIndex));
	};
	...
	private _whenSelectInstrument = (event: MouseEvent): void => {
		if (event.target == this._instrumentAddButton) {
			...
			if (index != -1) {
				Doc.selection.selectInstrument(index);
			}
			// Force piano to re-show, if channel is modulator
			if (Doc.channel >= Doc.song.pitchChannelCount + Doc.song.noiseChannelCount) {
				this._piano.forceRender();
	...
	private _whenSetModChannel = (mod: number): void => {
		...
		let previouslyUnset: boolean =
			instrument.modulators[mod] == 0 || Config.modulators[instrument.modulators[mod]].forSong;

		Doc.selection.setModChannel(mod, this._modChannelBoxes[mod].selectedIndex);
		...
		if (
			...
			if (Doc.song.channels[modChannel].bars[Doc.bar] > 0) {
				Doc.selection.setModInstrument(
					mod,
					Doc.song.channels[modChannel].patterns[Doc.song.channels[modChannel].bars[Doc.bar] - 1]
						.instruments[0]
				);
			}
		}
	...
	private _whenSetModInstrument = (mod: number): void => {
		Doc.selection.setModInstrument(mod, this._modInstrumentBoxes[mod].selectedIndex);
	...
	private _whenSetModSetting = (mod: number, invalidIndex: boolean = false): void => {
		...
		if (this._modSetBoxes[mod].selectedIndex != -1) {
			...
			if (invalidIndex) {
				...
				Doc.song.channels[Doc.channel].instruments[Doc.getCurrentInstrument()].invalidModulators[mod] = true;
			} else {
				this._modSetBoxes[mod].classList.remove("invalidSetting");
				Doc.song.channels[Doc.channel].instruments[Doc.getCurrentInstrument()].invalidModulators[mod] = false;
			}
		}
		if (!invalidIndex)
			// Invalid index means a set is actually not occurring, just the same index and a warning.
			Doc.selection.setModSetting(mod, text);
	...
	private _whenClickModTarget = (mod: number): void => {
		if (this._modChannelBoxes[mod].selectedIndex >= 2) {
			Doc.selection.setChannelBar(this._modChannelBoxes[mod].selectedIndex - 2, Doc.bar);
		}
	};

	private _whenClickJumpToModTarget = (): void => {
		const channelIndex: number = Doc.channel;
		const instrumentIndex: number = Doc.getCurrentInstrument();
		if (channelIndex < Doc.song.pitchChannelCount + Doc.song.noiseChannelCount) {
			for (
				let modChannelIdx: number = Doc.song.pitchChannelCount + Doc.song.noiseChannelCount;
				...
				if (patternIdx > 0) {
					...
					for (let mod: number = 0; mod < Config.modCount; mod++) {
						if (
							modInstrument.modChannels[mod] == channelIndex &&
							(modInstrument.modInstruments[mod] == instrumentIndex ||
								modInstrument.modInstruments[mod] >= Doc.song.channels[channelIndex].instruments.length)
						) {
							Doc.selection.setChannelBar(modChannelIdx, Doc.bar);
							return;
						}
					}
				}
	...
	private _whenSetModFilter = (mod: number): void => {
		Doc.selection.setModFilter(mod, this._modFilterBoxes[mod].selectedIndex);
	};

	private _whenSetChipWave = (): void => {
		Doc.record(new ChangeChipWave(Doc, this._chipWaveSelect.selectedIndex));
	};

	private _whenSetNoiseWave = (): void => {
		Doc.record(new ChangeNoiseWave(Doc, this._chipNoiseSelect.selectedIndex));
	};

	private _whenSetTransition = (): void => {
		Doc.record(new ChangeTransition(Doc, this._transitionSelect.selectedIndex));
	};
	...
	private _whenSetEffects = (): void => {
		...
		Doc.record(new ChangeToggleEffects(Doc, toggleFlag, null));
	...
	private _whenSetVibrato = (): void => {
		Doc.record(new ChangeVibrato(Doc, this._vibratoSelect.selectedIndex));
	};

	private _whenSetVibratoType = (): void => {
		Doc.record(new ChangeVibratoType(Doc, this._vibratoTypeSelect.selectedIndex));
	};

	private _whenSetUnison = (): void => {
		Doc.record(new ChangeUnison(Doc, this._unisonSelect.selectedIndex));
	};

	private _whenSetChord = (): void => {
		Doc.record(new ChangeChord(Doc, this._chordSelect.selectedIndex));
	};

	private _addNewEnvelope = (): void => {
		Doc.record(new ChangeAddEnvelope(Doc));
		this.refocusStage();
		Doc.addedEnvelope = true;
	};

	private _zoomIn = (): void => {
		Doc.prefs.visibleOctaves = Math.max(1, Doc.prefs.visibleOctaves - 1);
		Doc.prefs.save();
		Doc.notifier.changed();
		this.refocusStage();
	};
	...
	private _zoomOut = (): void => {
		Doc.prefs.visibleOctaves = Math.min(Config.pitchOctaves, Doc.prefs.visibleOctaves + 1);
		Doc.prefs.save();
		Doc.notifier.changed();
		this.refocusStage();
	};
	...
	private _fileMenuHandler = (event: Event): void => {
		switch (this._fileMenu.value) {
			...
			case "import":
				...
				break;
			case "copyUrl":
				this._copyTextToClipboard(new URL("#" + Doc.song.toBase64String(), location.href).href);
				break;
			case "shareUrl":
				(<any>navigator).share({ url: new URL("#" + Doc.song.toBase64String(), location.href).href });
				break;
			case "shortenUrl":
				window.open(
					"https://tinyurl.com/api-create.php?url=" +
						encodeURIComponent(new URL("#" + Doc.song.toBase64String(), location.href).href)
				);
				break;
			case "viewPlayer":
				location.href = "player/#song=" + Doc.song.toBase64String();
				break;
			case "copyEmbed":
				this._copyTextToClipboard(
					`<iframe width="384" height="60" style="border: none;" src="${
						new URL("player/#song=" + Doc.song.toBase64String(), location.href).href
					}"></iframe>`
				);
				break;
			case "songRecovery":
	...
	private _editMenuHandler = (event: Event): void => {
		switch (this._editMenu.value) {
			...
			case "pasteNumbers":
				...
				break;
			case "transposeUp":
				Doc.selection.transpose(true, false);
				break;
			case "transposeDown":
				Doc.selection.transpose(false, false);
				break;
			case "selectAll":
				Doc.selection.selectAll();
				break;
	...
	private _optionsMenuHandler = (event: Event): void => {
		switch (this._optionsMenu.value) {
			case "autoPlay":
				Doc.prefs.autoPlay = !Doc.prefs.autoPlay;
				break;
			case "autoFollow":
				Doc.prefs.autoFollow = !Doc.prefs.autoFollow;
				break;
			case "enableNotePreview":
				Doc.prefs.enableNotePreview = !Doc.prefs.enableNotePreview;
				break;
			case "showLetters":
				Doc.prefs.showLetters = !Doc.prefs.showLetters;
				break;
			case "showFifth":
				Doc.prefs.showFifth = !Doc.prefs.showFifth;
				break;
			case "notesOutsideScale":
				Doc.prefs.notesOutsideScale = !Doc.prefs.notesOutsideScale;
				break;
			case "setDefaultScale":
				Doc.prefs.defaultScale = Doc.song.scale;
				break;
			case "showChannels":
				Doc.prefs.showChannels = !Doc.prefs.showChannels;
				break;
			case "showScrollBar":
				Doc.prefs.showScrollBar = !Doc.prefs.showScrollBar;
				break;
			case "alwaysFineNoteVol":
				Doc.prefs.alwaysFineNoteVol = !Doc.prefs.alwaysFineNoteVol;
				break;
			case "enableChannelMuting":
				Doc.prefs.enableChannelMuting = !Doc.prefs.enableChannelMuting;
				for (const channel of Doc.song.channels) channel.muted = false;
				break;
			case "displayBrowserUrl":
				Doc.toggleDisplayBrowserUrl();
				break;
			case "displayVolumeBar":
				Doc.prefs.displayVolumeBar = !Doc.prefs.displayVolumeBar;
				break;
			case "layout":
	...
	private _customWavePresetHandler = (event: Event): void => {
		...
		Doc.record(new ChangeCustomWave(Doc, customWaveArray));
		Doc.record(
			new ChangeVolume(
				Doc,
				+this._instrumentVolumeSlider.input.value,
				-Config.volumeRange / 2 +
					Math.round((Math.sqrt(Config.chipWaves[index].expression) * Config.volumeRange) / 2)
			)
		);
