	...
	private _exportToMidi(): void {
		...
		for (const track of tracks) {
			...
			if (isMeta) {
				...
				let channelName: string = isNoise ? "noise channel " + channel : "pitch channel " + channel;
				writeEventTime(0);
				writer.writeUint8(MidiEventType.meta);
									...
									for (let toneIndex: number = 0; toneIndex < toneCount; toneIndex++) {
										...
										if (isDrumset) {
											...
											if (nextPitch < 0 || nextPitch >= drumsetMap.length)
												throw new Error(
													"Could not find corresponding drumset pitch. " + nextPitch
												);
											...
											if (
												...
												const midiTicksPerArpeggio =
													(Config.ticksPerArpeggio * midiTicksPerPart) / Config.ticksPerPart;
