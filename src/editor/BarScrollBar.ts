...
export class BarScrollBar {
	...
	public readonly container: HTMLElement = HTML.div(
		{ class: "barScrollBar", style: "width: 512px; height: 20px; overflow: hidden; position: relative;" },
		this._svg
	);
	...
	public render(): void {
		...
		if (resized) {
			...
			for (let i: number = 0; i <= this._doc.song.barCount; i++) {
				const lineHeight: number =
					i % 16 == 0 ? 0 : i % 4 == 0 ? this._editorHeight / 8 : this._editorHeight / 3;
		...
		this._doc.barScrollPos = Math.max(
			0,
			Math.min(this._doc.song.barCount - this._doc.trackVisibleBars, this._doc.barScrollPos)
		);
		this._doc.channelScrollPos = Math.max(
			0,
			Math.min(this._doc.song.getChannelCount() - this._doc.trackVisibleChannels, this._doc.channelScrollPos)
		);
