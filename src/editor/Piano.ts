...
export class Piano {
	...
	constructor(private _doc: SongDocument) {
		...
		for (let i: number = 0; i < Config.modCount; i++) {
			...
			const firstRowSVG: SVGSVGElement = SVG.svg(
				{ viewBox: "0 0 16 66", width: "16px", style: "pointer-events: none; flex-grow: 1;" },
				[firstRowText]
			);
			...
			const secondRowSVG: SVGSVGElement = SVG.svg(
				{ viewBox: "0 0 16 80", width: "16px", style: "pointer-events: none;" },
				[secondRowText]
			);
			...
			const modKey: HTMLDivElement = HTML.div(
				{ class: "modulator-button", style: "background: " + ColorConfig.modLabelPrimary + ";" },
				flexContainer
			);
	...
	private _onAnimationFrame = (): void => {
		...
		const liveInputPitchCount: number = !this._doc.performance.pitchesAreTemporary()
	...
	private _documentChanged = (): void => {
		...
		if (!isDrum && !isMod) {
			...
			for (let j: number = 0; j < Config.modCount; j++) {
				...
				if (instrument.modChannels[Config.modCount - j - 1] == -2) status = 0;
				else if (instrument.modChannels[Config.modCount - j - 1] == -1) status = 3;
				...
				switch (status) {
					...
					case 1:
						if (this._doc.song.channels[channelVal - 1].name == "") {
							...
							if (instrumentsLength > 1) {
								firstRow =
									"P" + channelVal + " " + this._doc.song.channels[channelVal - 1].name + insText;
							} else {
					...
					case 2:
						channelVal =
							instrument.modChannels[Config.modCount - j - 1] + 1 - this._doc.song.pitchChannelCount;
						...
						if (this._doc.song.channels[channelVal - 1].name == "") {
							...
							if (instrumentsLength > 1) {
								...
								firstRow =
									"N" + channelVal + " " + this._doc.song.channels[channelVal - 1].name + insText;
							} else {
				...
				if (this._doc.song.channels[Math.max(0, instrument.modChannels[Config.modCount - j - 1])].name != "") {
					...
					firstLabel.style.transform =
						...
						(-20 - squeeze - Math.round(Math.max(0, (height - 80) / 2))) +
						"px, 39px) scale(" +
						scaleFactor +
						", 1)";
					...
					firstLabel.style.transform =
						"rotate(-90deg) translate(" +
						(-20 - Math.round(Math.max(0, (height - 80) / 2))) +
						"px, 39px) scale(1, 1)";
				}
			}
		}
